...
const catBadge: Record<string, { color: string; label: string; icon: any }> = {
  A: { color: 'bg-emerald-500 text-white', label: 'A — No Risk (High Confidence)', icon: ShieldCheck },
  B: { color: 'bg-green-500 text-white', label: 'B — No Risk (Moderate)', icon: ShieldCheck },
  C: { color: 'bg-yellow-500 text-black', label: 'C — Low Confidence / Minor', icon: ShieldAlert },
  D: { color: 'bg-orange-500 text-white', label: 'D — Degradation Risk', icon: ShieldAlert },
...
const PRESETS: { label: string; desc: string; badge: string; data: FormData }[] = [
  {
    ...
    desc: 'Normal operating conditions — expect Category A',
    badge: 'A',
    data: {
      inverter_id: 'INV-DEMO-HEALTHY',
      dc_voltage: '650', dc_current: '72', ac_power: '48',
      module_temp: '38', ambient_temp: '32', irradiation: '920',
      alarm_code: '0', op_state: '5120', power_factor: '0.97', frequency: '50.02',
    },
  },
  {
    label: 'String Degradation',
    desc: 'Low current & power — expect Category C or D',
    badge: 'D',
    data: {
      inverter_id: 'INV-DEMO-DEGRAD',
      dc_voltage: '480', dc_current: '28', ac_power: '18',
      module_temp: '46', ambient_temp: '36', irradiation: '680',
  ...
  },
  {
    ...
    data: {
      ...
      dc_voltage: '520', dc_current: '45', ac_power: '12',
      module_temp: '72', ambient_temp: '48', irradiation: '600',
      alarm_code: '4003', op_state: '0', power_factor: '0.85', frequency: '50.05',
    },
  },
  {
    label: 'Grid Fault (Shutdown)',
    desc: 'Zero output, frequency deviation — expect Category E',
    badge: 'E',
    data: {
      inverter_id: 'INV-DEMO-GRIDFAULT',
      dc_voltage: '2', dc_current: '0.5', ac_power: '0.1',
      module_temp: '36', ambient_temp: '32', irradiation: '200',
      alarm_code: '5001', op_state: '0', power_factor: '0.35', frequency: '51.3',
    },
  },
];

const FIELDS = [
  { key: 'dc_voltage', label: 'DC Voltage', unit: 'V', min: 0, max: 2000, hint: 'Healthy: 580–720 V' },
  { key: 'dc_current', label: 'DC Current', unit: 'A', min: 0, max: 100, hint: 'Healthy: 55–85 A' },
  { key: 'ac_power', label: 'AC Power', unit: 'kW', min: 0, max: 500, hint: 'Healthy: 35–60 kW' },
  { key: 'module_temp', label: 'Module Temp', unit: '°C', min: -40, max: 150, hint: 'Healthy: 35–50°C' },
  { key: 'ambient_temp', label: 'Ambient Temp', unit: '°C', min: -40, max: 80, hint: 'Healthy: 28–42°C' },
  { key: 'irradiation', label: 'Irradiation', unit: 'W/m²', min: 0, max: 1500, hint: 'Healthy: 400–1000' },
] as const;
  ...
  const submit = async () => {
    ...
    try {
      const payload = {
        ...
        alarm_code: Number(form.alarm_code) || 0,
        op_state: Number(form.op_state) || 5120,
      ...
      const data = await chatbotApi.predictTest(payload);
      setResult(data.predictions?.[0] ?? null);
    } catch (err: any) {
      setError(err?.message || 'Prediction failed. Ensure GenAI and ML servers are running.');
    } finally {
      setLoading(false);
    }
  };
